// app/recommended-library/ClientList.tsx
'use client';
...
export default function ClientList({ initialRows }: { initialRows: Row[] }) {
  ...
  async function toggleFavorite(id: string, value: boolean) {
    await fetch('/api/recommended/favorite', {
      method: 'POST',
  ...
  async function bulkDelete() {
    ...
    await fetch('/api/recommended/bulk-delete', {
      method: 'POST',
      headers: { 'Content-Type':'application/json' },
  ...
  return (
    <div className="space-y-4">
      ...
      {!rows.length ? (
        <p className="text-sm text-gray-500">まだおすすめは保存されていません。おすすめ生成ページから保存してみてください。</p>
      ) : (
