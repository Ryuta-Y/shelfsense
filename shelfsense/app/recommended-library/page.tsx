...
export default async function RecommendedLibraryPage() {
  const { data, error } = await supabase
    ...
    .select('id, created_at, is_favorite, reason, books(*)')
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-3">
          <Link href="/library" className="text-sm text-blue-600 hover:underline">Library</Link>
          <Link href="/recommend" className="text-sm text-blue-600 hover:underline">おすすめ生成へ</Link>
          <Link href="/" className="text-sm text-blue-600 hover:underline">← ホームへ</Link>
        </div>
      </div>
